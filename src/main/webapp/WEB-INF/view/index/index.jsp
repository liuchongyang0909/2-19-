				...
				<ul class="list-group">
					<li class="channel-item  ${article.channelId==null?"active":"" }"><a href="/">推荐</a></li>
						<c:forEach items="${channels }" var="c">
							<li class="channel-item ${c.id==article.channelId?"active":""}"><a href="?channelId=${c.id }">${c.name }</a></li>
						</c:forEach> 
				</ul>
			</div>
			...
			<form action="/article/search" method="get">
				...
				<div>
					<ul class="subchannel">
						...
						<c:forEach items="${categorys}" var="c">
							<li class="sub-item ${c.id==article.categoryId?"sub-selected":"" }">
								<a class="nav-link" href="?channelId=${article.channelId}&categoryId=${c.id }">
									${c.name }
								</a>
							</li>
						</c:forEach>
				...
				<c:forEach items="${info.list }" var="a">
					...
					<div class="media" style="margin-top: 10px">
						  <a href="/article?id=${a.id}" target="_blank"><img src="/pic/${a.picture }" class="mr-3" alt="..." style="width: 150px" height="100px"></a>
						  <div class="media-body">
						    <h5 class="mt-0"><a href="/article?id=${a.id}" target="_blank">${a.title }</a></h5>
						    <h6 class="mt-0" style="margin-top: 3px">${a.user.username } 
				...
				<div class="card" style="width: 18rem; padding-left: 2px">
					...
					<c:forEach items="${lastInfo.list }" var="last">
						...
						<div class="class-body">
							<div class="media">
								<a href="/article?id=${last.id}" target="_blank">
									<img src="/pic/${last.picture }" class="mr-3" alt="..." style="height: 60px;width: 60px">
								</a>
								<div class="media-body">
